...
class SE(tf.keras.layers.Layer):
    ...
    def __init__(self, mconfig, se_filters, output_filters, name=None):
        ...
        self._se_reduce = tf.keras.layers.Conv2D(
            se_filters, kernel_size=1, strides=1, kernel_initializer=conv_kernel_initializer, padding="same", data_format=self._data_format, use_bias=True
        )
...
class MBConvBlock(tf.keras.layers.Layer):
    ...
    def __init__(self, block_args, mconfig, name=None):
        ...
        self._act = utils.get_act_fn(mconfig.act_fn)
        self._has_se = self._block_args.se_ratio is not None and 0 < self._block_args.se_ratio <= 1
    ...
    def _build(self):
        ...
        get_norm_name = lambda: self.name + "tpu_batch_normalization" + ("_0" if not next(bid) else "_" + str(next(bid) // 2))
        ...
        self._norm1 = utils.normalization(
            mconfig.bn_type, axis=self._channel_axis, momentum=mconfig.bn_momentum, epsilon=mconfig.bn_epsilon, groups=mconfig.gn_groups, name=get_norm_name()
        )

        if self._has_se:
            num_reduced_filters = max(1, int(self._block_args.input_filters * self._block_args.se_ratio))
            self._se = SE(self._mconfig, num_reduced_filters, filters)
        else:
            self._se = None
    ...
    def call(self, inputs, training, survival_prob=None):
        ...
        Returns:
          A output tensor.
        """
        logging.info("Block %s input shape: %s (%s)", self.name, inputs.shape, inputs.dtype)
        x = inputs
        if self._block_args.expand_ratio != 1:
...
class FusedMBConvBlock(MBConvBlock):
    ...
    def _build(self):
        ...
        if self._has_se:
            ...
            self._se = SE(mconfig, num_reduced_filters, filters)
        else:
            self._se = None
        # Output phase:
...
class Head(tf.keras.layers.Layer):
    ...
    def __init__(self, mconfig, name=None):
        ...
        self.h_axis, self.w_axis = [2, 3] if mconfig.data_format == "channels_first" else [1, 2]
...
class EffNetV2Model(tf.keras.Model):
    ...
    def __init__(self, model_name="efficientnetv2-s", model_config=None, include_top=True, num_classes=1000, name=None):
    ...
    def _build(self):
        ...
        for block_args in self._mconfig.blocks_args:
            ...
            repeats = round_repeats(block_args.num_repeat, self._mconfig.depth_coefficient)
            block_args.update(dict(input_filters=input_filters, output_filters=output_filters, num_repeat=repeats))
        ...
        if self.include_top and self.num_classes:
            self._fc = tf.keras.layers.Dense(
                self.num_classes, kernel_initializer=dense_kernel_initializer, bias_initializer=tf.constant_initializer(self._mconfig.headbias or 0)
            )
        else:
            self._fc = None
    ...
    def call(self, inputs, training, with_endpoints=False):
        ...
        # Calls Stem layers
        outputs = self._stem.call(inputs, training)
        ...
        for idx, block in enumerate(self._blocks):
            ...
            if (idx == len(self._blocks) - 1) or self._blocks[idx + 1].block_args.strides > 1:
                is_reduction = True
                reduction_idx += 1
...
def get_model(model_name, model_config=None, include_top=True, weights="imagenet", training=True, with_endpoints=False, **kwargs):
